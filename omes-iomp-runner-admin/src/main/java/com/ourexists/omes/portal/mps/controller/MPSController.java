...
public class MPSController {
    ...
    @Operation(summary = "删除", description = "已废弃：业务取消请走 POST /mo/adjust")
    @PostMapping("delete")
    public JsonResponseEntity<Boolean> delete(@Validated @RequestBody IdsDto idsDto) {
        throw new BusinessException("${mps.msg.delete.use.adjust}");
    }

    @Operation(summary = "改变优先级", description = "delegating → /mo/adjust PRIORITY")
    @PostMapping("changePriority")
    public JsonResponseEntity<Boolean> changePriority(@Validated @RequestBody ChangePriorityDto dto) {
        adjustPriority("REORDER", dto.getCurrent(), dto.getPre(), dto.getPost());
    ...
    private MoAdjustResult adjustPriority(String mode, String mpsId, String pre, String post) {
        if (StringUtils.isBlank(mpsId)) {
            throw new BusinessException("${mo.adjust.mpsIds.required}");
        }
        MPSDto mps;
        try {
            mps = RemoteHandleUtils.getDataFormResponse(mpsFeign.selectById(mpsId));
        } catch (EraCommonException e) {
            throw new BusinessException(e.getMessage());
        }
        if (mps == null || StringUtils.isBlank(mps.getMoCode())) {
            throw new BusinessException("${mo.adjust.mps.notfound}");
        }
        Map<String, Object> payload = new HashMap<>();
        payload.put("mode", mode);
        payload.put("mpsId", mpsId);
        payload.put("current", mpsId);
        payload.put("pre", pre);
        payload.put("post", post);
        MoAdjustCommand cmd = new MoAdjustCommand()
                .setMoCode(mps.getMoCode())
                .setAdjustType(MoAdjustTypeEnum.PRIORITY.name())
                .setSource(MoAdjustSourceEnum.UI.name())
                .setRequestId(IdWorker.getIdStr())
                .setPayload(payload);
        return moAdjustOrchestrator.adjust(cmd);
    }
}
