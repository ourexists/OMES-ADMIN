...
@Slf4j
@Component("StoreWinCCMag")
public class StoreWinCCMagTimerTask extends TimerTask {
    ...
    @Override
    public void doRun() {
        MagDevVari datalist = cacheUtils.get("realtime", WinCCDevConstants.MAG_CACHE);
        if (datalist == null) {
            return;
        }
        try {
            RemoteHandleUtils.getDataFormResponse(winCCReportFeign.saveMag(MagDevVari.covert(datalist)));
