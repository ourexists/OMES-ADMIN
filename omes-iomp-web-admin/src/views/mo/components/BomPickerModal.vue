...
import { fetchBomPage } from '@/api/bom'
...
const columns = computed(() => [
  { title: t('moPage.bomName'), dataIndex: 'name', key: 'name', ellipsis: true },
  { title: t('moPage.bomCode'), dataIndex: 'selfCode', key: 'selfCode', width: 140, align: 'center' as const },
  { title: t('bomPage.type'), dataIndex: 'typeDesc', key: 'typeDesc', width: 120, align: 'center' as const },
...
function onSelect(record: BomRecord) {
  emit('select', record)
        ...
        >
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'action'">
              <a-button type="link" size="small" @click="onSelect(record as BomRecord)">
                <CheckOutlined />
