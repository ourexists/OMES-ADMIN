...
public interface MPSService extends IMyBatisPlusService<MPS> {
    ...
    /**
     * 作废 MPS。forceExecing=true 时允许中止 EXECING（COMMON/EXEC→STOP，MPS→CANCEL，不走 FILE/push）。
     */
    List<MPS> voidMpsCascade(Collection<String> ids, boolean forceExecing);

    default List<MPS> voidMpsCascade(Collection<String> ids) {
        return voidMpsCascade(ids, false);
    }

    void updateExecTime(Collection<String> ids, java.util.Date execTime);

    /**
     * 更新未开工批次明细设备绑定。
     */
    void rebindDev(String mpsId, String matCode, String devNo, String devName);
}
