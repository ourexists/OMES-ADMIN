...
package com.ourexists.mesedge.portal.wincc;
...
public class WinccApi {
    ...
    public List<WinccVariable> pullTags(List<WinccVariable> variables) {
        ConnectDto connect = connect();
        String url = getUri(connect) + TAG_PATH;
        Map<String, List<String>> params = Maps.newHashMap();
        List<String> variableNames = variables.stream().map(WinccVariable::getAttrName).collect(Collectors.toList());
        ...
        if (resp.getStatusCode() == HttpStatus.OK) {
            ...
            for (int i = 0; i < jsonArray.size(); i++) {
                ...
                for (WinccVariable variable : variables) {
                    if (variable.getAttrName().equals(field)) {
