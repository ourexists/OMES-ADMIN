...
const AGENT_PREFIX = `${BASE_URL}/inspection/ai/agent`;
...
function buildTargetUrl(path: string[], requestUrl: string): string {
  ...
  const target = new URL(`${AGENT_PREFIX}/${joinedPath}`);
  target.search = incoming.search;
  return target.toString();
}

async function forward(method: string, req: Request, context: RouteContext): Promise<Response> {
  const targetUrl = buildTargetUrl(context.params.path, req.url);
  const hasBody = method !== "GET";
  const body = hasBody ? await req.text() : undefined;
  const headers = buildForwardHeaders(req, hasBody);
